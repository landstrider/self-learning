var repoArr = [
 ...
 let myMap = new Map();
 let mappedRepos = function(reposArr) {
   reposArr.forEach(function(arrayObjItem) {
     for (let name in arrayObjItem) {
       if(arrayObjItem.hasOwnProperty(name)) {
         myMap.set(arrayObjItem.name, arrayObjItem);
       }
     }
   })
 }  
mappedRepos(repoArr);
//console.log(myMap.get("react-basic-pluralsight"));

let reqRepoDataArr = (function(array, keys) {
  	  return array.map(o => Object.assign(...keys.map(k => ({
				[k] : o[k]
			  }))))	
  	})(repoArr, ['name', 'description', 'updated_at', 'language', 'stargazers_count']);
